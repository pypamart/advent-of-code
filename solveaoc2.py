...
def test_solution(year, day):
    try:
        test_file = f"aoc/year_{year}/day_{day:02d}/test_day_{day:02d}_solvers.py"
...
def solve_advent_problem(year, day, part):
    try:
        module_name = f"aoc.year_{year}.day_{day:02d}.day_{day:02d}_solvers"
        print(module_name)
        ...
        if part == 1:
            ...
            print(f"Invalid part number {part}.")
            sys.exit(1)
