...
if __name__ == "__main__":
    diskmap = read_input("input_1.txt")
    ...
    for pos, value in enumerate(diskmap):
        ...
        blocks.append([item, int(value)])    

    # print("".join([ str(value) * nb for value, nb in blocks]))

    # Step 2: sort the block
    stacks = defaultdict(list)
    slots = []
    for block in blocks:
        if block[0] != ".":
            stacks[block[1]].append(block)
            slots.append((block[1], True))
        else:
            slots.append((block[1], False))
    ...
    for slot in slots:
        if slot[1]:
            ...
            if block is None:
                sorted_blocks.append((".", gap_size))

        print(sorted_blocks)
        print(" ".join([ str(value) * nb for value, nb in sorted_blocks]))
