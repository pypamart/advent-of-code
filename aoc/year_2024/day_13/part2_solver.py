...
def parse_machine_instruction(instructions: str) -> tuple:
    ...
    configuration["XP"] = int(instructions[2].split("X=")[1].split(",")[0]) + 10000000000000
    configuration["YP"] = int(instructions[2].split("Y=")[1]) + 10000000000000
...
def solve(filepath: Path=INPUT_FILE.absolute()) -> int:
    ...
    for machine in machines:
        ...
        if result is None:
            continue
        n_a, n_b = result
        if int(n_a) == n_a and int(n_b) == n_b:
